...
<table>
<tr>
<td align="center" width="25%"><strong>수신</strong><br><sub>SMTP · LMTP<br>alias · Sieve</sub></td>
<td align="center" width="25%"><strong>접근</strong><br><sub>IMAP · POP3<br>JMAP · push</sub></td>
<td align="center" width="25%"><strong>발송</strong><br><sub>MX · 스마트호스트<br>큐 · DKIM</sub></td>
...
| 필요한 것 | 처음 시작할 구성 | 나중에 추가할 것 |
| --- | --- | --- |
| 로컬 개발 서버 | SQLite + 파일시스템 블롭 + 자체서명 TLS | IMAP, JMAP, 관리 API |
| 단일 운영 노드 | SQLite 또는 PostgreSQL + 파일/S3 블롭 | ACME, 메트릭, 감사, 워커 |
| 여러 메일 노드 | PostgreSQL/MySQL + 공용 S3 호환 블롭 | 리스너별 TLS, 역할별 워커 |

### 기본값부터 안전하게

| 경계 | 기본 동작 |
| --- | --- |
| 저장 중 비밀값 | master key가 없으면 시작하지 않으며, 평문 저장은 명시적으로 켜야 함 |
| 선택 리스너 | 포트를 지정하기 전에는 비활성 |
| 평문 인증 | TLS를 사용할 수 없으면 차단 |
| 관리 API | HTTP method 기준 scope 검사 적용 |
...
서버는 npm 패키지인 `dotenv`를 자동으로 읽지 않습니다. Node.js 24의 내장 `--env-file` 기능을 사용하면 별도 의존성 없이 같은 방식으로 구성할 수 있습니다.
...
IONOSPHERE_MASTER_KEY=길고-무작위인-비밀값으로-교체
IONOSPHERE_HOSTNAME=mail.example.com
IONOSPHERE_TLS_MODE=selfsigned
IONOSPHERE_SMTP_STARTTLS=1
IONOSPHERE_SMTP_PORT=2525
...
`.env`에는 DB 비밀번호, API token, private key, master key가 들어갈 수 있으므로 버전 관리에 포함하지 마십시오. 로컬 개발용이라면 `.gitignore`에도 추가하는 것이 안전합니다.
