...
**`main`에 머지해도 배포되지 않는다.** 커밋은 쌓이고, 내보낼 시점은 사람이 정한다 —
Actions의 **Release & deploy**를 한 번 실행하면 그 커밋의 CI가 초록인지 확인하고
태그와 릴리즈를 만든 뒤 배포 신호를 보낸다.

실제 서버 배포는 **별도의 비공개 저장소**가 그 태그를 받아 수행한다. 배포 대상 주소·호스트
지문·자격증명이 거기 있기 때문이다 — 이 저장소는 공개라 Actions 로그도 공개이고, 로그에 한 줄
찍히면 되돌릴 수 없다(시크릿으로 옮겨도 부족하다: GitHub는 시크릿의 *정확한 문자열*만 가리므로
거기서 잘라낸 IP는 그대로 노출된다).

> 이 저장소의 히스토리는 2026-08-23에 새로 시작했다. 그 전 이력에 운영 자격증명이 평문으로
> 들어간 적이 있어 가져오지 않았다. 코드 자체는 그때까지의 모든 작업이 반영된 상태다.
