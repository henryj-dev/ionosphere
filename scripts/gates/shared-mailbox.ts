...
const GATES: Record<string, Phase> = {
  ...
  P5: {
    needs: ["P4"],
    outputs: ["apps/server/src/jmap-server.ts", "apps/server/src/jmap-backend.ts", "apps/server/src/principal-context.ts", "apps/server/test/jmap-shared-account.test.ts", "packages/store/src/store.ts", "packages/store/src/types.ts", "packages/store/src/index.ts", "packages/store/test/authorization.test.ts"],
    checks: [
      ...
      { id: "G-P5.7", kind: "command", command: ["npm", "run", "lint"] },
      { id: "G-P5.8", kind: "command", command: ["npm", "run", "typecheck"] },
    ],
  },
