...
const GATES: Record<string, Phase> = {
  ...
  P5: {
    needs: ["P4"],
    outputs: ["apps/server/src/jmap-server.ts", "apps/server/src/principal-context.ts", "packages/store/src/store.ts", "packages/store/src/types.ts", "packages/store/src/index.ts", "packages/store/test/authorization.test.ts"],
    checks: [
      { id: "G-P5.1", kind: "command", command: ["node", "--test", "packages/store/test/authorization.test.ts"] },
      { id: "G-P5.2", kind: "grep", path: "apps/server/src/jmap-server.ts", pattern: "listAccessibleAccounts" },
      { id: "G-P5.3", kind: "grep", path: "apps/server/src/jmap-server.ts", pattern: "isReadOnly" },
      { id: "G-P5.4", kind: "grep", path: "packages/store/src/store.ts", pattern: "listAccessibleAccounts" },
      { id: "G-P5.5", kind: "command", command: ["npm", "run", "lint"] },
      { id: "G-P5.6", kind: "command", command: ["npm", "run", "typecheck"] },
    ],
  },
