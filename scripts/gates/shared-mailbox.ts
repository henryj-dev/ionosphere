...
const GATES: Record<string, Phase> = {
  ...
  P4: {
    ...
    checks: [
      ...
      { id: "G-P4.3", kind: "grep", path: "packages/proto-imap/src/engine.ts", pattern: "GETACL" },
      { id: "G-P4.4", kind: "grep", path: "packages/store/src/authorization.ts", pattern: "setMailboxAcl" },
      { id: "G-P4.5", kind: "grep", path: "packages/store/src/authorization.ts", pattern: "deleteMailboxAcl" },
      { id: "G-P4.6", kind: "command", command: ["npm", "run", "lint"] },
      { id: "G-P4.7", kind: "command", command: ["npm", "run", "typecheck"] },
    ],
  },
