...
describe("앱 비밀번호 인증", () => {
  test("생성 → 그 비번으로 인증 성공(하이픈 포함/제거/공백 모두)", async () => {
    ...
    expect(await authenticate(db, "u@x.test", password, "imap")).toMatchObject({ accountId }); // 원본(하이픈 포함)
    const compact = password.replace(/-/g, "");
    expect(await authenticate(db, "u@x.test", compact, "imap")).toMatchObject({ accountId }); // 하이픈 제거
    expect(await authenticate(db, "u@x.test", password.replace(/-/g, " "), "imap")).toMatchObject({ accountId }); // 공백
  });
  ...
  test("기본 비번도 여전히 동작, 틀린 앱비번은 실패", async () => {
    ...
    expect(await authenticate(db, "u@x.test", "wxyz-wxyz-wxyz-wxyz", "imap")).toBeNull();
  });
...
describe("목록·폐기", () => {
  ...
  test("폐기 → 인증 불가, 기본 비번은 폐기 대상 아님", async () => {
    ...
    expect(await authenticate(db, "u@x.test", password, "imap")).toBeNull();
